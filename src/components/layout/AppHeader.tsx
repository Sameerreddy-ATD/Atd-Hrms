...
import { Bell, Home, LogOut, User as UserIcon, Search, Settings, HelpCircle, Menu, Sun, Moon, Monitor, Check } from "lucide-react";
    ...
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-4 bg-[#F6F8FC]/40 dark:bg-zinc-950/45 backdrop-blur-md border-b border-slate-200/20 dark:border-zinc-900/30 px-4 md:px-6">
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 mt-1 rounded-xl shadow-md border border-border/40 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md">
            <DropdownMenuLabel className="font-semibold text-sm px-3 py-2">
              ...
              <div className="text-xs text-muted-foreground font-normal">{user ? ROLE_LABELS[user.role] : ""}</div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            ...
            <DropdownMenuSub>
              ...
              <DropdownMenuPortal>
                <DropdownMenuSubContent className="w-36 rounded-xl shadow-md border border-border/40 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md p-1">
                  <DropdownMenuItem onClick={() => handleThemeChange("light")} className="cursor-pointer flex items-center justify-between">
                    <span className="flex items-center"><Sun className="mr-2 h-4 w-4 text-amber-500" /> Light</span>
                    {activeTheme === "light" && <Check className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleThemeChange("dark")} className="cursor-pointer flex items-center justify-between">
                    <span className="flex items-center"><Moon className="mr-2 h-4 w-4 text-blue-400" /> Dark</span>
                    {activeTheme === "dark" && <Check className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleThemeChange("system")} className="cursor-pointer flex items-center justify-between">
                    <span className="flex items-center"><Monitor className="mr-2 h-4 w-4 text-slate-500 dark:text-zinc-400" /> System</span>
                    {activeTheme === "system" && <Check className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />}
                  </DropdownMenuItem>
                </DropdownMenuSubContent>
              </DropdownMenuPortal>
            </DropdownMenuSub>
