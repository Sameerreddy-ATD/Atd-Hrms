...
export function StatCard({
  ...
  return (
    <Card
      className={cn(
        "card-hover group relative overflow-hidden border-border/70 bg-card/95 shadow-xs backdrop-blur-sm",
        className,
      )}
    >
      ...
      <CardContent className="h-full px-4 py-4 sm:px-5 sm:py-4.5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            {hint && <p className="mt-1.5 text-xs text-muted-foreground/90">{hint}</p>}
          </div>
          {Icon && (
            <div
              className={cn(
