...
npm run start:backend > /tmp/e2e-backend.log 2>&1 &
echo $! > /tmp/e2e-backend.pid
npm run preview -- --host localhost --port 4173 > /tmp/e2e-frontend.log 2>&1 &
...
E2E_SKIP_INFRA=1 E2E_BASE_URL=http://localhost:4173 E2E_API_BASE_URL=http://localhost:4000 \
  npx playwright test tests/e2e/attendance-exceptions.spec.ts --project=desktop-chromium
EC=$?
