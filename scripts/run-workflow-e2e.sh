#!/usr/bin/env bash
# Task Planner workflow authenticated E2E on disposable MySQL (port 3308).
set -euo pipefail
...
npm run start:backend > /tmp/e2e-workflow-backend.log 2>&1 &
echo $! > /tmp/e2e-workflow-backend.pid
npm run preview -- --host localhost --port 4173 > /tmp/e2e-workflow-frontend.log 2>&1 &
echo $! > /tmp/e2e-workflow-frontend.pid
...
npx playwright test tests/e2e/task-planner-workflow.spec.ts --project=desktop-chromium --reporter=line
STATUS=$?

kill "$(cat /tmp/e2e-workflow-backend.pid)" "$(cat /tmp/e2e-workflow-frontend.pid)" 2>/dev/null || true
exit $STATUS
