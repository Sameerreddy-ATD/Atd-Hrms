#!/usr/bin/env bash
# Task Planner search + saved views authenticated E2E on disposable MySQL (port 3308).
set -euo pipefail
...
npm run start:backend > /tmp/e2e-search-backend.log 2>&1 &
echo $! > /tmp/e2e-search-backend.pid
npm run preview -- --host localhost --port 4173 > /tmp/e2e-search-frontend.log 2>&1 &
echo $! > /tmp/e2e-search-frontend.pid
...
npx playwright test tests/e2e/task-planner-search.spec.ts --project=desktop-chromium --reporter=line
STATUS=$?

kill "$(cat /tmp/e2e-search-backend.pid)" "$(cat /tmp/e2e-search-frontend.pid)" 2>/dev/null || true
exit $STATUS
