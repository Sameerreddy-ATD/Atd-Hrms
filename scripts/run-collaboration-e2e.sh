#!/usr/bin/env bash
# Task Planner collaboration authenticated E2E on disposable MySQL (port 3308).
set -euo pipefail
...
npm run start:backend > /tmp/e2e-collab-backend.log 2>&1 &
echo $! > /tmp/e2e-collab-backend.pid
npm run preview -- --host localhost --port 4173 > /tmp/e2e-collab-frontend.log 2>&1 &
echo $! > /tmp/e2e-collab-frontend.pid
