...
| C2 | People / Profile | `/profile` · post-punch verification | **Superseded** — employees confirm fields after check-in; HR reviews **Profile Corrections**. The unused `POST /profile/edit-requests` stub remains. | | | | Resolved by verification flow |
